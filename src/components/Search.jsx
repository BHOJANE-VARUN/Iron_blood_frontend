...
function Search() {
  ...
  return (
    <div className="w-screen h-screen bg-rose-50 flex flex-col  items-center px-4">
      <div className="fixed inset-1 bg-white text-black shadow-md rounded-md px-4 py-3 flex items-center justify-between h-16 z-50">
        <p className="text-sm sm:text-base">
          This website is currently hosted on a{" "}
          <span className="font-medium text-blue-600">free-tier server</span>,
          which may cause slight latency in API responses.
          <br className="sm:hidden" />
          View a demo on{" "}
          <a
            href="https://youtu.be/vQ5ASYkn5S8"
          ...
          , or run it locally using the source code from{" "}
          <a
            href="https://github.com/BHOJANE-VARUN/Iron_blood_frontend"
            className="text-blue-500 underline hover:text-blue-700 transition"
            target="_blank"
            rel="noopener noreferrer"
          >
      ...
      <h1 className="text-center text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-gray-800 leading-tight mt-56 mb-5">
        Find the right <span className="text-rose-500">philanthropist</span>
        <br />
        for your needs
      </h1>
      ...
      <div className="w-full max-w-xl relative">
        <div className="flex border border-gray-300 rounded-xl shadow-md overflow-hidden bg-white">
          <input
            ...
            type="text"
            className="w-full text-gray-800 px-5 py-3 focus:outline-none placeholder-gray-400 text-lg"
            placeholder="Search hospital by name. e.g., 'Apollo Hospital'"
            onChange={(e) => {
              settemp(null);
              setSearchTerm(e.target.value);
            }}
